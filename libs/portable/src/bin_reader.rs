...
mod tests {
    ...
    fn parses_verifies_and_extracts_archive() {
        let data = archive(
            &[
                ("bin/camellia.exe", b"portable executable"),
                ("data/empty.txt", b""),
            ],
            "bin/camellia.exe",
        );
        ...
        assert_eq!(reader.exe, Path::new("bin/camellia.exe"));
        ...
        assert_eq!(
            fs::read(directory.join("bin/camellia.exe")).unwrap(),
    ...
    fn rejects_unsafe_and_duplicate_paths() {
        ...
        let duplicate = archive(
            &[("camellia.exe", b"first"), ("camellia.exe", b"second")],
            "camellia.exe",
        );
    ...
    fn rejects_truncated_trailing_and_missing_executable_data() {
        let data = archive(&[("camellia.exe", b"payload")], "camellia.exe");
        ...
        let missing = archive(&[("other.exe", b"payload")], "camellia.exe");
        assert!(BinaryReader::read(&missing).is_err());
    }

    #[test]
    fn rejects_corrupt_content_before_extraction() {
        let mut data = archive(&[("camellia.exe", b"payload")], "camellia.exe");
