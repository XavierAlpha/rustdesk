...
class PortableGeneratorTests(unittest.TestCase):
    ...
    def test_archive_and_metadata_are_deterministic(self) -> None:
        (self.source / "camellia.exe").write_bytes(b"executable")
        ...
        self.assertEqual(
            ...
            ["camellia.exe", "z.txt", "assets/a.txt"],
        )

        executable = GENERATOR.resolve_executable(source, "camellia.exe")
    ...
    def test_executable_must_be_a_regular_file_inside_source(self) -> None:
        executable = self.source / "camellia.exe"
        ...
        self.assertEqual(
            GENERATOR.resolve_executable(source, "camellia.exe"),
            "camellia.exe",
        )
