...
def _manifest_version(root: Path) -> str:
    ...
    if not isinstance(package, dict) or package.get("name") != "rustdesk":
        raise MetadataError("Cargo.toml must describe package rustdesk")
...
def _lock_version(root: Path) -> str:
    ...
    matches = [
        ...
        if isinstance(package, dict) and package.get("name") == "rustdesk"
    ]
    if len(matches) != 1 or not isinstance(matches[0].get("version"), str):
        raise MetadataError("Cargo.lock must contain exactly one rustdesk package")
