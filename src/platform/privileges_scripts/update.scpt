on run {daemon_file, agent_file, user, cur_pid, source_dir}
  ...
  set agent_plist to "/Library/LaunchAgents/com.carriez.camellia_server.plist"
  set daemon_plist to "/Library/LaunchDaemons/com.camellia_service.plist"
  set app_bundle to "/Applications/Camellia.app"
  ...
  set kill_others to "pids=$(pgrep -x 'Camellia' | grep -vx " & cur_pid & " || true); if [ -n \"$pids\" ]; then echo \"$pids\" | xargs kill -9 || true; fi;"
