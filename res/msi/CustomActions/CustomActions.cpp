...
UINT __stdcall CustomActionHello(__in MSIHANDLE hInstall) {
  HRESULT hr = S_OK;
...
// Helper function to safely delete a file or directory using handle-based
// deletion. This avoids TOCTOU (Time-Of-Check-Time-Of-Use) race conditions.
BOOL SafeDeleteItem(LPCWSTR fullPath) {
  ...
  // Use shared access to allow deletion even when other processes have the file
  // open.
  DWORD flags = FILE_FLAG_BACKUP_SEMANTICS | FILE_FLAG_OPEN_REPARSE_POINT;
  HANDLE hFile = CreateFileW(fullPath, DELETE,
                             FILE_SHARE_READ | FILE_SHARE_WRITE |
                                 FILE_SHARE_DELETE, // Allow shared access
                             NULL, OPEN_EXISTING, flags, NULL);

  if (hFile == INVALID_HANDLE_VALUE) {
    WcaLog(LOGMSG_STANDARD, "SafeDeleteItem: Failed to open '%ls'. Error: %lu",
  ...
  BOOL result = SetFileInformationByHandle(hFile, FileDispositionInfo,
                                           &dispInfo, sizeof(dispInfo));

  if (!result) {
    DWORD error = GetLastError();
    WcaLog(LOGMSG_STANDARD,
           "SafeDeleteItem: Failed to mark '%ls' for deletion. Error: %lu",
           fullPath, error);
  }
...
// Helper function to recursively delete a directory's contents with detailed
// logging.
void RecursiveDelete(LPCWSTR path) {
  ...
  if (PathIsRootW(path)) {
    WcaLog(LOGMSG_STANDARD,
           "RecursiveDelete: refusing to operate on root path '%ls'.", path);
    return;
  }

  // MAX_PATH is enough here since the installer should not be using longer
  // paths. No need to handle extended-length paths (\\?\) in this context.
  WCHAR searchPath[MAX_PATH];
  HRESULT hr = StringCchPrintfW(searchPath, MAX_PATH, L"%s\\*", path);
  if (FAILED(hr)) {
    WcaLog(LOGMSG_STANDARD, "RecursiveDelete: Path too long to enumerate: %ls",
           path);
    return;
  }
  ...
  if (hFind == INVALID_HANDLE_VALUE) {
    // This can happen if the directory is empty or doesn't exist, which is not
    // an error in our case.
    WcaLog(LOGMSG_STANDARD,
           "RecursiveDelete: Failed to enumerate directory '%ls'. It may be "
           "missing or inaccessible. Error: %lu",
           path, GetLastError());
    return;
  }
  ...
  do {
    ...
    hr = StringCchPrintfW(fullPath, MAX_PATH, L"%s\\%s", path,
                          findData.cFileName);
    if (FAILED(hr)) {
      WcaLog(
          LOGMSG_STANDARD,
          "RecursiveDelete: Path too long for item '%ls' in '%ls', skipping.",
    ...
    if (findData.dwFileAttributes & FILE_ATTRIBUTE_READONLY) {
      if (FALSE == SetFileAttributesW(fullPath, findData.dwFileAttributes &
                                                    ~FILE_ATTRIBUTE_READONLY)) {
        WcaLog(
            LOGMSG_STANDARD,
            "RecursiveDelete: Failed to remove read-only attribute. Error: %lu",
            GetLastError());
      }
    }

    if (findData.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY) {
      // Check for reparse points (symlinks/junctions) to prevent directory
      // traversal attacks. Do not follow reparse points, only remove the link
      // itself.
      if (findData.dwFileAttributes & FILE_ATTRIBUTE_REPARSE_POINT) {
        WcaLog(LOGMSG_STANDARD,
               "RecursiveDelete: Not recursing into reparse point "
               "(symlink/junction), deleting link itself: %ls",
               fullPath);
  ...
  if (lastError != ERROR_NO_MORE_FILES) {
    WcaLog(LOGMSG_STANDARD,
           "RecursiveDelete: FindNextFileW failed with error %lu", lastError);
  }
...
//   5. InstallExecute
//   6. InstallFinalize
UINT __stdcall RemoveInstallFolder(__in MSIHANDLE hInstall) {
  HRESULT hr = S_OK;
  DWORD er = ERROR_SUCCESS;
  ...
  ExitOnFailure(hr, "failed to read database key from custom action data: %ls",
                pwz);

  if (installFolder == NULL || installFolder[0] == L'\0') {
    WcaLog(LOGMSG_STANDARD,
           "Install folder path is empty, skipping recursive delete.");
    goto LExit;
  }

  if (PathIsRootW(installFolder)) {
    WcaLog(LOGMSG_STANDARD, "Refusing to recursively delete root folder '%ls'.",
           installFolder);
    goto LExit;
  }

  WcaLog(LOGMSG_STANDARD,
         "Attempting to recursively delete contents of install folder: %ls",
         installFolder);

  RecursiveDelete(installFolder);

  // The standard MSI 'RemoveFolders' action will take care of removing the (now
  // empty) directories. We don't need to call RemoveDirectoryW on installFolder
  // itself, as it might still be in use by the installer.
...
// **NtQueryInformationProcess** may be altered or unavailable in future
// versions of Windows. Applications should use the alternate functions listed
// in this topic. But I do not find the alternate functions.
...
typedef NTSTATUS(NTAPI *pfnNtQueryInformationProcess)(HANDLE, PROCESSINFOCLASS,
                                                      PVOID, ULONG, PULONG);
bool TerminateProcessIfNotContainsParam(
    pfnNtQueryInformationProcess NtQueryInformationProcess, HANDLE process,
    LPCWSTR excludeParam) {
  bool processClosed = false;
  PROCESS_BASIC_INFORMATION processInfo;
  NTSTATUS status =
      NtQueryInformationProcess(process, ProcessBasicInformation, &processInfo,
                                sizeof(processInfo), NULL);
  if (status == 0 && processInfo.PebBaseAddress != NULL) {
    PEB peb;
    SIZE_T dwBytesRead;
    if (ReadProcessMemory(process, processInfo.PebBaseAddress, &peb,
                          sizeof(peb), &dwBytesRead)) {
      RTL_USER_PROCESS_PARAMETERS pebUpp;
      if (ReadProcessMemory(process, peb.ProcessParameters, &pebUpp,
                            sizeof(RTL_USER_PROCESS_PARAMETERS),
                            &dwBytesRead)) {
        if (pebUpp.CommandLine.Length > 0) {
          // Allocate extra space for null terminator
          WCHAR *commandLine =
              (WCHAR *)malloc(pebUpp.CommandLine.Length + sizeof(WCHAR));
          if (commandLine != NULL) {
            ...
            if (ReadProcessMemory(process, pebUpp.CommandLine.Buffer,
                                  commandLine, pebUpp.CommandLine.Length,
                                  &dwBytesRead)) {
...
bool TerminateProcessesByNameW(LPCWSTR processName, LPCWSTR excludeParam) {
  ...
  if (NtQueryInformationProcess == NULL) {
    WcaLog(LOGMSG_STANDARD,
           "Failed to get address of NtQueryInformationProcess.");
  }
      ...
      do {
        if (lstrcmpW(processName, processEntry.szExeFile) == 0) {
          ...
          if (process != NULL) {
            if (NtQueryInformationProcess == NULL) {
              WcaLog(LOGMSG_STANDARD,
                     "Terminate process : %ls, while NtQueryInformationProcess "
                     "is NULL",
                     processName);
              TerminateProcess(process, 0);
              processClosed = true;
              ...
              processClosed = TerminateProcessIfNotContainsParam(
                  NtQueryInformationProcess, process, excludeParam);
            }
            CloseHandle(process);
          }
        }
...
UINT __stdcall TerminateProcesses(__in MSIHANDLE hInstall) {
  HRESULT hr = S_OK;
  DWORD er = ERROR_SUCCESS;
...
void AddFirewallRuleCmdline(LPWSTR exeName, LPWSTR exeFile, LPCWSTR dir) {
  ...
  StringCchPrintfW(rulename, sizeof(rulename) / sizeof(rulename[0]),
                   L"%ls Service", exeName);
  if (FAILED(hr)) {
  ...
  StringCchPrintfW(cmdline, sizeof(cmdline) / sizeof(cmdline[0]),
                   L"advfirewall firewall add rule name=\"%ls\" dir=%ls "
                   L"action=allow program=\"%ls\" enable=yes",
                   rulename, dir, exeFile);
  if (FAILED(hr)) {
  ...
  if ((int)hi <= 32) {
    WcaLog(LOGMSG_STANDARD,
           "Failed to change firewall rule : %d, last error: %d", (int)hi,
           GetLastError());
  } else {
    WcaLog(LOGMSG_STANDARD, "Firewall rule \"%ls\" (%ls) is added", rulename,
           dir);
  }
}
...
void RemoveFirewallRuleCmdline(LPWSTR exeName) {
  ...
  StringCchPrintfW(cmdline, sizeof(cmdline) / sizeof(cmdline[0]),
                   L"advfirewall firewall delete rule name=\"%ls\"", rulename);
  if (FAILED(hr)) {
    WcaLog(LOGMSG_STANDARD, "Failed to make cmdline: %ls", exeName);
    return;
  }
  ...
  if ((int)hi <= 32) {
    WcaLog(LOGMSG_STANDARD,
           "Failed to change firewall rule \"%ls\" : %d, last error: %d",
           rulename, (int)hi, GetLastError());
  } else {
    WcaLog(LOGMSG_STANDARD, "Firewall rule \"%ls\" is removed", rulename);
  }
}

UINT __stdcall AddFirewallRules(__in MSIHANDLE hInstall) {
  HRESULT hr = S_OK;
  DWORD er = ERROR_SUCCESS;
  ...
  ExitOnFailure(hr, "failed to read database key from custom action data: %ls",
                pwz);
  WcaLog(LOGMSG_STANDARD, "Try add firewall exceptions for file : %ls",
         exeFile);
...
UINT __stdcall SetPropertyIsServiceRunning(__in MSIHANDLE hInstall) {
  ...
  WcaLog(LOGMSG_STANDARD, "Try set is service running, property name : \"%ls\"",
         szPropertyName);
...
void TryCreateStartServiceByShell(LPWSTR svcName, LPWSTR svcBinary,
                                  LPWSTR szSvcDisplayName);
UINT __stdcall CreateStartService(__in MSIHANDLE hInstall) {
  ...
  DWORD cchSvcDisplayName =
      sizeof(szSvcDisplayName) / sizeof(szSvcDisplayName[0]);
  ...
  if (IsServiceRunningW(svcName)) {
    ...
    WcaLog(
        LOGMSG_STANDARD,
        "Service \"%ls\" is not running, try create and start service by shell",
        svcName);
...
UINT __stdcall TryStopDeleteService(__in MSIHANDLE hInstall) {
  ...
  if (MyStopServiceW(svcName)) {
    ...
    WcaLog(LOGMSG_STANDARD, "Failed to stop service: \"%ls\", error: 0x%02X.",
           svcName, GetLastError());
  }

  if (IsServiceRunningW(svcName)) {
    WcaLog(LOGMSG_STANDARD, "Service \"%ls\" is not stoped after 1000 ms.",
           svcName);
  } else {
  ...
  if (MyDeleteServiceW(svcName)) {
    WcaLog(LOGMSG_STANDARD,
           "Service \"%ls\" deletion is completed without errors.", svcName);
  } else {
    WcaLog(LOGMSG_STANDARD, "Failed to delete service: \"%ls\", error: 0x%02X.",
           svcName, GetLastError());
  }

  if (QueryServiceStatusExW(svcName, &svcStatus)) {
    WcaLog(LOGMSG_STANDARD,
           "Failed to delete service: \"%ls\", current status: %d.", svcName,
    ...
    if (lastErrorCode == ERROR_SERVICE_DOES_NOT_EXIST) {
      ...
      WcaLog(LOGMSG_STANDARD,
             "Failed to query service status: \"%ls\", error: 0x%02X.", svcName,
             lastErrorCode);
      TryStopDeleteServiceByShell(svcName);
    }
  }
  ...
  // But the upgrading may be stucked at "copying new files" because the file is
  // in using. Steps to reproduce: Install -> stop service in tray --> start
  // service -> upgrade Sleep(300);
...
UINT __stdcall TryDeleteStartupShortcut(__in MSIHANDLE hInstall) {
  ...
  WcaLog(LOGMSG_STANDARD, "Try delete startup shortcut of : \"%ls\"",
         szShortcut);

  hr =
      StringCchPrintfW(pwszTemp, 1024, L"%ls%ls.lnk", szStartupDir, szShortcut);
  ExitOnFailure(hr, "Failed to compose a resource identifier string");

  if (DeleteFileW(pwszTemp)) {
    WcaLog(LOGMSG_STANDARD, "Failed to delete startup shortcut of : \"%ls\"",
           pwszTemp);
  } else {
...
UINT __stdcall SetPropertyFromConfig(__in MSIHANDLE hInstall) {
  ...
  WcaLog(LOGMSG_STANDARD, "Try read configuration, config key : \"%ls\"",
         szConfigKey);

  MsiGetPropertyW(hInstall, L"PropertyName", szPropertyName, &cchPropertyName);
  WcaLog(LOGMSG_STANDARD, "Try read configuration, property name : \"%ls\"",
         szPropertyName);
...
UINT __stdcall AddRegSoftwareSASGeneration(__in MSIHANDLE hInstall) {
  ...
  hi = ShellExecuteW(
      ...
      L" add "
      L"HKEY_LOCAL_"
      L"MACHINE\\Software\\Microsoft\\Windows\\CurrentVersion\\Policies\\System"
      L" /f /v SoftwareSASGeneration /t REG_DWORD /d 1",
      NULL, SW_HIDE);
  // https://learn.microsoft.com/en-us/windows/win32/api/shellapi/nf-shellapi-shellexecutew
  if ((int)hi <= 32) {
    WcaLog(LOGMSG_STANDARD, "Failed to add registry name \"%ls\", %d, %d",
           valueName, (int)hi, GetLastError());
  } else {
  ...
  if (result != ERROR_SUCCESS) {
    WcaLog(LOGMSG_STANDARD, "Failed to create or open registry key: %d",
           result);
    goto LExit;
  }

  result =
      RegSetValueExW(hKey, valueName, 0, valueType,
                     reinterpret_cast<const BYTE *>(valueData), valueDataSize);
  if (result != ERROR_SUCCESS) {
...
UINT __stdcall RemoveAmyuniIdd(__in MSIHANDLE hInstall) {
  HRESULT hr = S_OK;
  DWORD er = ERROR_SUCCESS;
  ...
  if (fileAttributes == INVALID_FILE_ATTRIBUTES) {
    WcaLog(LOGMSG_STANDARD, "Amyuni idd dir \"%ls\" is not found, %d", workDir,
           fileAttributes);
    goto LExit;
  }
  ...
  // https://learn.microsoft.com/en-us/windows/win32/api/shellapi/nf-shellapi-shellexecutew
  if ((int)hi <= 32) {
    WcaLog(LOGMSG_STANDARD, "Failed to remove amyuni idd : %d, last error: %d",
           (int)hi, GetLastError());
  } else {
...
void TryCreateStartServiceByShell(LPWSTR svcName, LPWSTR svcBinary,
  ...
  WcaLog(LOGMSG_STANDARD, "TryCreateStartServiceByShell, service: %ls",
         svcName);
  ...
  i = 0;
  j = 0;
  // svcBinary is a string with double quotes, we need to escape it for shell
  // arguments. It is orignal used for `CreateServiceW`. eg. "C:\Program
  // Files\MyApp\MyApp.exe" --service -> \"C:\Program Files\MyApp\MyApp.exe\"
  // --service
  while (true) {
    if (svcBinary[j] == L'"') {
      szNewBin[i] = L'\\';
      ...
      if (i >= cchNewBin) {
        WcaLog(LOGMSG_STANDARD,
               "Failed to copy bin for service: %ls, buffer is not enough",
               svcName);
        return;
      }
      szNewBin[i] = L'"';
  ...
  hr = StringCchPrintfW(
      szCmd, cchCmd,
      L"create %ls binpath= \"%ls\" start= auto DisplayName= \"%ls\"", svcName,
      szNewBin, szSvcDisplayName);
  if (FAILED(hr)) {
  ...
  if ((int)hi <= 32) {
    WcaLog(LOGMSG_STANDARD,
           "Failed to create service with shell : %d, last error: 0x%02X.",
           (int)hi, GetLastError());
  } else {
    ...
    if (!QueryServiceStatusExW(svcName, &svcStatus)) {
      ...
      if (lastErrorCode == ERROR_SERVICE_DOES_NOT_EXIST) {
        if (k == 29) {
          WcaLog(
              LOGMSG_STANDARD,
              "Failed to query service status: \"%ls\", service is not found.",
              svcName);
          return;
        } else {
          Sleep(100);
  ...
  if ((int)hi <= 32) {
    WcaLog(LOGMSG_STANDARD,
           "Failed to start service with shell : %d, last error: 0x%02X.",
           (int)hi, GetLastError());
  } else {
...
void TryStopDeleteServiceByShell(LPWSTR svcName) {
  ...
  if (!QueryServiceStatusExW(svcName, &svcStatus)) {
    ...
    WcaLog(LOGMSG_STANDARD,
           "Failed to query service status: \"%ls\" with shell, error: 0x%02X.",
           svcName, lastErrorCode);
  } else {
    WcaLog(LOGMSG_STANDARD,
           "Status of service: \"%ls\" with shell, current status: %d.",
           svcName, svcStatus.dwCurrentState);
  }
  ...
  if ((int)hi <= 32) {
    WcaLog(LOGMSG_STANDARD,
           "Failed to delete service with shell : %d, last error: 0x%02X.",
           (int)hi, GetLastError());
  } else {
    WcaLog(LOGMSG_STANDARD,
           "Service \"%ls\" deletion is completed without errors with shell,",
           svcName);
  }
  ...
  for (int k = 0; k < 10; ++k) {
    if (!QueryServiceStatusExW(svcName, &svcStatus)) {
      if (GetLastError() == ERROR_SERVICE_DOES_NOT_EXIST) {
        WcaLog(LOGMSG_STANDARD, "Service \"%ls\" is deleted with shell.",
               svcName);
        return;
      }
    }
  ...
  if (!QueryServiceStatusExW(svcName, &svcStatus)) {
    ...
    WcaLog(LOGMSG_STANDARD,
           "Failed to delete service: \"%ls\" with shell, current status: %d.",
           svcName, svcStatus.dwCurrentState);
  }
}

UINT __stdcall InstallPrinter(__in MSIHANDLE hInstall) {
  HRESULT hr = S_OK;
  DWORD er = ERROR_SUCCESS;
...
UINT __stdcall UninstallPrinter(__in MSIHANDLE hInstall) {
  HRESULT hr = S_OK;
  DWORD er = ERROR_SUCCESS;
